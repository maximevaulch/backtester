...
class StrategyA30min(BaseStrategy):
    """
    An implementation of the 30-minute Order Block / FVG strategy.
    """
    ...
    def _log_event(self, timestamp, event_type, details, leg=None, zone=None):
    ...
    def _find_fvg_near_index(self, df, origin_idx, fvg_type, tf_cols):
    ...
    def _perform_zone_creation_logic(self, leg, violation_idx, df_30min, tf_cols, fvg_type, timestamp):
        origin_candle_idx = leg['origin_candle_idx']
        check_name, fvg_indices = self._find_fvg_near_index(df_30min, origin_candle_idx, fvg_type, tf_cols)
        if not fvg_indices:
            self._log_event(timestamp, 'FVG_CHECK_FAILED', f"No {fvg_type} FVG found near origin idx {origin_candle_idx}.", leg=leg)
            return None
        self._log_event(timestamp, 'FVG_CHECK_PASSED', f"Found {fvg_type} FVG via rule: '{check_name}'.", leg=leg)
        ...
        if is_mitigated:
            self._log_event(timestamp, 'ZONE_DISCARDED', "Reason: Zone was already mitigated.", leg=leg, zone={'zone_low': zone_low, 'zone_high': zone_high})
            return None
        self._log_event(timestamp, 'TRADING_ZONE_DEFINED', "Zone is fresh and confirmed.", leg=leg, zone={'zone_low': zone_low, 'zone_high': zone_high})
        return {'type': fvg_type, 'zone_low': zone_low, 'zone_high': zone_high}

    def generate_conditions(self, df: pd.DataFrame, strategy_params: dict = {}) -> pd.DataFrame:
        self.event_log = []
        ...
        for i, row in enumerate(df_30min.itertuples(name='Candle')):
            ...
            if new_vl_confirmed:
                ...
                if last_confirmed_vh:
                    ...
                    all_legs.append(new_leg); self._log_event(row.Index, 'LEG_FORMED', f"Bearish leg from {new_leg['high']:.5f} to {new_leg['low']:.5f}", leg=new_leg)
            ...
            if new_vh_confirmed:
                ...
                if last_confirmed_vl:
                    ...
                    all_legs.append(new_leg); self._log_event(row.Index, 'LEG_FORMED', f"Bullish leg from {new_leg['low']:.5f} to {new_leg['high']:.5f}", leg=new_leg)
            ...
            for leg in all_legs:
                if leg['status'] == 'pending_fvg_check' and i >= leg['check_at_idx']:
                    self._log_event(row.Index, 'PENDING_CHECK_EXECUTED', "Executing delayed FVG check.", leg=leg)
                    zone_info = self._perform_zone_creation_logic(leg, leg['violation_idx'], df_30min, tf_cols, 'BEARISH' if leg['type'] == 'BULLISH' else 'BULLISH', row.Index)
                    ...
                    if is_violated:
                        self._log_event(row.Index, 'LEG_VIOLATED', f"Leg violated by close price {row_close:.5f}.", leg=leg)
                        violation_idx, origin_idx = i, leg['origin_candle_idx']; idx_diff = violation_idx - origin_idx
                        if idx_diff >= 3:
                            self._log_event(row.Index, 'FVG_CHECK_TRIGGERED', f"Violation distance is {idx_diff} (>=3), checking immediately.", leg=leg)
                            zone_info = self._perform_zone_creation_logic(leg, violation_idx, df_30min, tf_cols, 'BEARISH' if leg['type'] == 'BULLISH' else 'BULLISH', row.Index)
                            if zone_info: active_trading_zones.append(zone_info)
                            leg['status'] = 'processed'
                        else:
                            ...
                            self._log_event(row.Index, 'FVG_CHECK_DELAYED', f"Delaying check until index {leg['check_at_idx']}.", leg=leg)
                ...
                for exec_row in exec_df.itertuples():
                    for zone in list(active_trading_zones):
                        ...
                        if zone['type'] == 'BEARISH' and getattr(exec_row, exec_high_col) >= zone['zone_low']:
                            ...
                            entry_triggered = True
                        if entry_triggered:
                            self._log_event(exec_row.Index, 'ENTRY_SIGNAL_GENERATED', f"Entry triggered on {zone['type']} zone.", zone=zone)
        ...
        if self.event_log:
            ...
            debug_df = pd.DataFrame(self.event_log, columns=log_columns).sort_values(by='timestamp').reset_index(drop=True)
